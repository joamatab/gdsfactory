...
class OasisWriteSettings(BaseModel):
    ...
    detect_rectangles: bool = Field(
        default=True,
        description="If true, stores rectangles in a compressed format.",
    )
    detect_trapezoids: bool = Field(
        default=True,
        description="If true, stores trapezoids in a compressed format.",
    )
...
class Pdk(BaseModel):
    ...
    def validate_layers(self, layers_required: list[Layer] | None = None):
        ...
        for layer in layers_required:
            if layer not in self.layers:
                msg = f"{layer!r} not in Pdk.layers {list(self.layers.keys())}"
                raise ValueError(
                    msg,
                )
    ...
    def register_cells(self, **kwargs) -> None:
        ...
        for name, cell in kwargs.items():
            if not callable(cell):
                msg = f"{cell} is not callable, make sure you register cells functions that return a Component"
                raise ValueError(
                    msg,
                )
    ...
    def register_cross_sections(self, **kwargs) -> None:
        ...
        for name, cross_section in kwargs.items():
            if not callable(cross_section):
                msg = f"{cross_section} is not callable, make sure you register cross_section functions that return a CrossSection"
                raise ValueError(
                    msg,
                )
        ...
        **kwargs,
        ...
        if dirpath:
            ...
            if not dirpath.is_dir():
                msg = f"{dirpath!r} needs to be a directory."
        ...
        for k, v in kwargs.items():
            if not update and k in self.cells:
                msg = f"ERROR: Cell name {k!r} already registered."
                raise ValueError(msg)
            self.cells[k] = v
    ...
    def remove_cell(self, name: str):
        ...
        if name not in self.cells:
            msg = f"{name!r} not in {list(self.cells.keys())}"
            raise ValueError(msg)
        self.cells.pop(name)
    ...
    def get_cell(self, cell: CellSpec, **kwargs) -> ComponentFactory:
        ...
        if callable(cell):
            ...
            if cell not in cells:
                ...
                msg = f"{cell!r} from PDK {self.name!r} not in cells: {cells} "
                raise ValueError(
                    msg,
                )
            ...
            for key in cell:
                if key not in component_settings:
                    msg = f"Invalid setting {key!r} not in {component_settings}"
                    raise ValueError(
                        msg,
                    )
            ...
            if not isinstance(cell_name, str) or cell_name not in cells:
                cells = list(self.cells.keys())
                msg = f"{cell_name!r} from PDK {self.name!r} not in cells: {cells} "
                raise ValueError(
                    msg,
                )
            ...
            msg = f"get_cell expects a CellSpec (ComponentFactory, string or dict),got {type(cell)}"
            raise ValueError(
                msg,
            )
    ...
    def _get_component(
        ...
        **kwargs,
        ...
        if isinstance(component, Component):
            if kwargs:
                msg = f"Cannot apply kwargs {kwargs} to {component.name!r}"
                raise ValueError(msg)
            return component
            ...
            if component not in cells:
                msg = f"{component!r} not in PDK {self.name!r} cells: {cells} "
                raise ValueError(
                    msg,
                )
            ...
            msg = f"get_component expects a ComponentSpec (Component, ComponentFactory, string or dict), got {type(component)}"
            raise ValueError(
                msg,
            )
        ...
        **kwargs,
        ...
        if isinstance(cross_section, CrossSection):
            ...
            if (
                ...
                cross_sections = list(self.cross_sections.keys())
                msg = f"{cross_section_factory_name!r} not in {cross_sections}"
                raise ValueError(
                    msg,
                )
            ...
            msg = f"get_cross_section expects a CrossSectionSpec (CrossSection, CrossSectionFactory, Transition, string or dict), got {type(cross_section)}"
            raise ValueError(
                msg,
            )
    ...
    def get_layer(self, layer: LayerSpec) -> Layer:
        ...
        if isinstance(layer, tuple | list):
            if len(layer) != 2:
                msg = f"{layer!r} needs two integer numbers."
                raise ValueError(msg)
            return layer
            ...
            if layer not in self.layers:
                msg = f"{layer!r} not in {self.layers.keys()}"
                raise ValueError(msg)
            ...
            msg = f"{layer!r} needs to be a LayerSpec (string, int or Layer)"
            raise ValueError(
                msg,
            )
    ...
    def get_layer_views(self) -> LayerViews:
        if self.layer_views is None:
            msg = f"layer_views for Pdk {self.name!r} is None"
            raise ValueError(msg)
        return self.layer_views

    def get_layer_stack(self) -> LayerStack:
        if self.layer_stack is None:
            msg = f"layer_stack for Pdk {self.name!r} is None"
            raise ValueError(msg)
    ...
    def get_material_index(self, key: str, *args, **kwargs) -> float:
        if key not in self.materials_index:
            ...
            msg = f"{key!r} not in {material_names}"
            raise ValueError(msg)
...
def get_sparameters_path() -> pathlib.Path:
    ...
    if PDK.sparameters_path is None:
        msg = f"{_ACTIVE_PDK.name!r} has no sparameters_path"
        raise ValueError(msg)
...
def get_interconnect_cml_path() -> pathlib.Path:
    ...
    if PDK.interconnect_cml_path is None:
        msg = f"{_ACTIVE_PDK.name!r} has no interconnect_cml_path"
        raise ValueError(msg)
